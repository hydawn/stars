...
#	endif // STARS_PLATFORM_LINUX
		 ...
		 << "  test <number>       test the game for <number> times, default "
			"1, number must\n"
		 << "                      follow the -test option closely, option "
			"test can be used\n"
			...
			if (!MainArgsHandle::inVector(argl, argv[2])) { // must be a number
				try {
					...
					throw invalid_argument(
						"argsHandle: wrong usage of option test");
				}
			}
			if (rounds < 1)
...
#	endif // STARS_DEBUG_INFO
	cout << "Enter number to place a piece, \"help\" for help, \"exit\" to "
			"exit.\n";
#else
#	ifdef STARS_DEBUG_INFO
	cout << "一个调试版本Stars\n";
